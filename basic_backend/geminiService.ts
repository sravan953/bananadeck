...
export const generateSlideStructure = async (urls: string[]): Promise<Presentation> => {
    const prompt = `You are an expert presentation designer, focusing on visual storytelling. Based on the content from the provided URLs, generate a cohesive slide deck structure.
    ...
    try {
        // Create parts array with the prompt and URL contexts
        const parts = [
            { text: prompt },
            ...urls.map(url => ({ url }))
        ];
